...
原因：Caddy 为 localhost 签发的是自签名证书，不被系统信任。
...
7. 使用系统工具 logrotate 管理日志（推荐生产环境）
创建 /etc/logrotate.d/caddy：
...
这样即使 Caddy 自身不配置轮转，系统也会自动处理。

8. 总结
Caddy 的最大优势是自动 HTTPS 和简洁的配置语法。

遇到 403 错误时，优先检查文件权限，并确保网站根目录对 caddy 用户可读。

独立日志配置很有用，但注意日志目录的权限问题。

使用 caddy reload 可以实现零停机配置更新，配合 caddy fmt 保持格式规范。

本文档基于实际操作整理，适用于 Caddy v2。如有更多需求，建议查阅 Caddy 官方文档。
